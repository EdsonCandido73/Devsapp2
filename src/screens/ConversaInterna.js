...
import { Modal, View, Text, Platform, StyleSheet, KeyboardAvoidingView, TouchableHighlight, Image, BackHandler, TextInput, FlatList } from 'react-native';
			...
			if(r.uri) {
				...
				.then((blob)=>{
					...
					this.props.sendImage(
						blob,
						(snapshot)=>{
							let pct = ( snapshot.bytesTranferred / snapshot.totalBytes ) * 100;
		...
		return(
			<KeyboardAvoidingView style={styles.container} behavior={AreaBehavior} keyboardVerticalOffset={AreaOffset}>
				<FlatList 
				 ...
				 <Modal animationType="slide" tranparent={false} visible={this.state.modalVisible}>
				 	<TouchableHighlight style={styles.modalView} onPress={()=>{ this.setModalVisible(false) }}>
				 		<Image resizerMode="contain" style={styles.modalImage} source={{uri:this.state.modalImage}} />
				 	</TouchableHighlight>
				 </Modal>	
...
const ConversasInternaConnect = connect(mapStateToProps, { setActiveChat, sendMessage, monitorChat, monitorChatOff, sendImage })(ConversaInterna);
export default ConversasInternaConnect;